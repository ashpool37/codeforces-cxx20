...
// Problem statement: https://codeforces.com/problemset/problem/509/A?locale=en
...
ull table_value(unsigned const x, unsigned const y) {
    if(x == 0u or y == 0u) return 1u;
    return table_value(x - 1u, y) + table_value(x, y - 1u);
}

int main() {
    unsigned const table_size = from_cin();
    std::cout << table_value(table_size - 1u, table_size - 1u) << std::endl;
}
