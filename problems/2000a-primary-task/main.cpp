...
// Problem statement: https://codeforces.com/problemset/problem/2000/A?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        std::string number = line_from_cin();
        bool potentially_important =
            number.size() >= 3
            and number[0] == '1'
            and number[1] == '0'
            and ((number[2] == '1' and number.size() >= 4)
                 or (digit_to_unsigned(number[2]) >= 2));
        std::cout << (potentially_important ? "YES" : "NO") << std::endl;
    }
}
