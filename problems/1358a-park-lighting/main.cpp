...
// Problem statement: https://codeforces.com/problemset/problem/1358/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const park_width = from_cin();
        unsigned const park_height = from_cin();
        unsigned const lamp_post_count = [park_width, park_height]() {
            if(park_width % 2u == 0u or park_height % 2u == 0u)
                return park_width * park_height / 2u;
            else return (park_width * park_height + 1u) / 2u;
        }();
        std::cout << lamp_post_count << std::endl;
    }
}
