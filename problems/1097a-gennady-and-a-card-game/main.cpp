...
// Problem statement: https://codeforces.com/problemset/problem/1097/A?locale=en
...
std::istream& operator>>(std::istream& is, Card& card) {
    ...
    is >> card.rank >> card.suit;
    return is;
}

bool can_make_move_p(Card const& table_card, Card const& card) {
    return table_card.rank == card.rank or table_card.suit == card.suit;
}

int main() {
    Card const table_card = from_cin();
    std::array<Card, 5> const hand = from_cin();
    bool const can_make_move =
        std::ranges::any_of(hand,
                [table_card](Card const& card) { return can_make_move_p(table_card, card); });
    std::cout << (can_make_move ? "YES" : "NO") << std::endl;
}
