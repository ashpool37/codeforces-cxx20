...
// Problem statement: https://codeforces.com/problemset/problem/459/B?locale=en
...
std::size_t equal_prefix_size(R&& range) {
    ...
    do {
        result++;
        it++;
    } while(it != std::ranges::end(range) and *it == value);
    return result;
}
...
int main() {
    unsigned const flower_count = from_cin();
    std::vector<unsigned> const flowers = [flower_count]() {
        std::vector<unsigned> result = from_cin(flower_count);
    ...
    std::cout << beauty_difference << " " << number_of_ways << std::endl;
}
