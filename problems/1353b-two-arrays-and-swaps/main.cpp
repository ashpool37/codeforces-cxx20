...
// Problem statement: https://codeforces.com/problemset/problem/1353/B?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned array_length = from_cin();
        unsigned turn_count_max = from_cin();
        std::vector<unsigned> numbers_a = from_cin(array_length);
        std::vector<unsigned> numbers_b = from_cin(array_length);
        for(auto _ : counted(turn_count_max)) {
            auto min_a = std::ranges::min_element(numbers_a);
            auto max_b = std::ranges::max_element(numbers_b);
            if(*min_a < *max_b)
                std::iter_swap(min_a, max_b);
            else break;
        }
        std::cout << std::accumulate(numbers_a.begin(), numbers_a.end(), 0u) << std::endl;
    }
}
