// Problem: 1999C. Showering
// Problem statement: https://codeforces.com/problemset/problem/1999/C?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned const task_count = from_cin();
        unsigned const showering_duration = from_cin();
        unsigned const day_duration = from_cin();
        ...
        unsigned const task_count_including_phony_ones = task_count + 2u;
        tasks.reserve(task_count_including_phony_ones);
        tasks.emplace_back(0u, 0u);
        for(auto _ : counted(task_count)) tasks.emplace_back(from_cin(), from_cin());
        tasks.emplace_back(day_duration, day_duration);

        enum CanShowerResult {
            CAN_SHOWER,
            CANNOT_SHOWER,
        };
        auto const can_shower = [task_count_including_phony_ones, showering_duration,
                                 &tasks = std::as_const(tasks)] () {
            for(auto task_id : counted(task_count_including_phony_ones - 1u)) {
                auto const task = tasks[task_id];
                auto const next_task = tasks[task_id + 1u];
                if(next_task.start_time - task.end_time >= showering_duration)
                    return CanShowerResult::CAN_SHOWER;
            }
            return CanShowerResult::CANNOT_SHOWER;
        };
        std::cout << (can_shower() == CanShowerResult::CAN_SHOWER ? "YES" : "NO") << std::endl;
    }
}
