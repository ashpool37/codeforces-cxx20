...
// Problem statement: https://codeforces.com/problemset/problem/1520/D?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        ...
        std::unordered_map<long, umax> const counts_by_number_minus_index = [number_count]() {
            std::unordered_map<long, umax> result;
            for(unsigned const index : counted(number_count)) {
                unsigned const number = from_cin();
                long const number_minus_index = static_cast<long>(number) - static_cast<long>(index);
                result[number_minus_index]++;
            }
            return result;
        }();

        umax eligible_pair_count = 0;
        for(auto const [_, count] : counts_by_number_minus_index) {
            eligible_pair_count += choose2(count);
        }

        std::cout << eligible_pair_count << std::endl;
    }
}
