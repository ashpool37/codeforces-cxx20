...
// Problem statement: https://codeforces.com/problemset/problem/1399/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const gift_count = from_cin();
        std::vector<unsigned> const candy_counts = from_cin(gift_count);
        std::vector<unsigned> const orange_counts = from_cin(gift_count);
        unsigned const target_candy_count = std::ranges::min(candy_counts);
        unsigned const target_orange_count = std::ranges::min(orange_counts);
        auto const moves_to_equalize = [target_candy_count, target_orange_count,
                                        &candy_counts, &orange_counts]
                (std::size_t gift_number) -> ull {
            ull const candies_to_eat_count = candy_counts[gift_number] - target_candy_count;
            ull const oranges_to_eat_count = orange_counts[gift_number] - target_orange_count;
            ull const eat_both_count = std::min(candies_to_eat_count, oranges_to_eat_count);
            ull const eat_one_count = std::max(candies_to_eat_count, oranges_to_eat_count)
                                        - eat_both_count;
            return eat_both_count + eat_one_count;
        };
        ull const moves_to_equalize_total =
            sum(std::views::transform(counted(gift_count), moves_to_equalize));
        std::cout << moves_to_equalize_total << std::endl;
    }
}