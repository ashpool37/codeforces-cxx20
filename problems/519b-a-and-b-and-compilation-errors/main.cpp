...
// Problem statement: https://codeforces.com/problemset/problem/519/B?locale=en
...
int main() {
    unsigned const initial_error_count = from_cin();
    std::vector<unsigned> errors_fixed;
    std::unordered_multiset<unsigned> errors;
    std::copy_n(std::istream_iterator<unsigned>(std::cin), initial_error_count, std::inserter(errors, errors.begin()));

    std::unordered_multiset<unsigned> errors_fixed_1 = errors;
    for(auto const _ : counted(initial_error_count - 1u)) {
        unsigned const error = from_cin();
        errors_fixed_1.erase(errors_fixed_1.find(error));
    }
    errors_fixed.push_back(*errors_fixed_1.begin());

    std::unordered_multiset<unsigned> errors_fixed_2 = errors;
    errors_fixed_2.erase(errors_fixed_2.find(errors_fixed[0]));
    for(auto const _ : counted(initial_error_count - 2u)) {
        unsigned const error = from_cin();
        errors_fixed_2.erase(errors_fixed_2.find(error));
    }
    errors_fixed.push_back(*errors_fixed_2.begin());

    std::cout << errors_fixed[0] << "\n" << errors_fixed[1] << std::endl;
}
