...
// Problem statement: https://codeforces.com/problemset/problem/1343/C?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const sequence_length = from_cin();
        std::vector<int> const sequence = from_cin(sequence_length);
        ...
        auto const sign = [](int const number) {
            if(number == 0) return Sign::ZERO;
            else if(number < 0) return Sign::MINUS;
            else return Sign::PLUS;
        };

        int local_max = sequence[0];
        ll subsequence_sum = 0;
        for(int const number : sequence | std::views::drop(1)) {
            if(sign(number) == sign(local_max))
                local_max = std::max(local_max, number);
            else {
                subsequence_sum += local_max;
                local_max = number;
            }
        }
        subsequence_sum += local_max;

        std::cout << subsequence_sum << std::endl;
    }
}
