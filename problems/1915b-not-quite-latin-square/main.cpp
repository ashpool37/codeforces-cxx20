...
// Problem statement: https://codeforces.com/problemset/problem/1915/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        for(auto const _ : counted(3uz)) {
            std::string const line = line_from_cin();
            if(line.contains('?')) {
                if(not line.contains('A')) std::cout << "A" << std::endl;
                if(not line.contains('B')) std::cout << "B" << std::endl;
                if(not line.contains('C')) std::cout << "C" << std::endl;
            }
        }
    }
}
