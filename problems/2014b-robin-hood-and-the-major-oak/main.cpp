...
// Problem statement: https://codeforces.com/problemset/problem/2014/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const target_year = from_cin();
        unsigned const leaf_period = from_cin();
        bool const even = (target_year * leaf_period - (leaf_period * (leaf_period - 1u)) / 2u) % 2u == 0u;
