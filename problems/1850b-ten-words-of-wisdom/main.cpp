...
// Problem statement: https://codeforces.com/problemset/problem/1850/B?locale=en
...
std::istream& operator>>(std::istream& input_stream, Response& response) {
    input_stream >> response.word_count >> response.quality;
    return input_stream;
}
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const response_count = from_cin();
        std::vector<Response> responses = from_cin(response_count);
        auto responses_w_less_than_10_words = std::views::zip(responses, std::views::iota(1))
            | std::views::filter([](auto r) { return std::get<0>(r).word_count <= 10u; });
        unsigned const max_response_id = std::get<1>(std::ranges::max(responses_w_less_than_10_words,
            [](auto const& lhs, auto const& rhs) { return std::get<0>(lhs).quality < std::get<0>(rhs).quality; }));
        std::cout << max_response_id << std::endl;
    }
}