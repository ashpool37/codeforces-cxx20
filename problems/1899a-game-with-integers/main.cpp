...
// Problem statement: https://codeforces.com/problemset/problem/1899/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const initial_number = from_cin();
        enum class Player {
            VANYA,
            VOVA,
        };
        Player const winner = initial_number % 3u == 0u
            ? Player::VOVA
            : Player::VANYA;
        std::cout << (winner == Player::VANYA ? "First" : "Second") << std::endl;
    }
}
