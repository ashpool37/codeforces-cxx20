...
// Problem statement: https://codeforces.com/problemset/problem/584/A?locale=en
...
std::optional<std::string> make_number(unsigned digit_count, unsigned divisor) {
    if(divisor > 10u or divisor < 1u) return std::nullopt;
    if(divisor == 10u) {
        if(digit_count < 2u) return std::nullopt;
        return std::string("1") + (std::views::repeat('0')
                                   | std::views::take(digit_count - 1u)
                                   | std::ranges::to<std::string>());
    }
    else {
        return std::views::repeat(unsigned_to_digit(divisor))
               | std::views::take(digit_count)
               | std::ranges::to<std::string>();
    }
}

int main() {
    unsigned const digit_count = from_cin();
    unsigned const divisor = from_cin();
    std::cout << make_number(digit_count, divisor).value_or("-1") << std::endl;
}
