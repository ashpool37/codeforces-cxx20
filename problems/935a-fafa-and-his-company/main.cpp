...
// Problem statement: https://codeforces.com/problemset/problem/935/A?locale=en
...
unsigned divisor_count(unsigned dividend) {
    unsigned result = 0u;
    for(unsigned divisor = 1u; divisor * divisor <= dividend; divisor++) {
        auto div_result = std::div(static_cast<long>(dividend), static_cast<long>(divisor));
        if(div_result.rem == 0l) {
            if(divisor == div_result.quot) result += 1u;
            else [[likely]] result += 2u;
        }
    }
    return result;
}
...
int main() {
    unsigned const employee_count = from_cin();
    std::cout << divisor_count(employee_count) - 1u << std::endl;
}
