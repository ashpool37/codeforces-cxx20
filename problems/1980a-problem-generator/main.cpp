...
// Problem statement: https://codeforces.com/problemset/problem/1980/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const problem_bank_size = from_cin();
        unsigned const round_count = from_cin();
        cin_skip_line();
        auto const problem_bank = [problem_bank_size]() {
            std::unordered_map<char, unsigned> result;
            for(auto const problem_difficulty : {'A', 'B', 'C', 'D', 'E', 'F', 'G'}) {
                result[problem_difficulty] = 0u;
            }
            for(auto const _ : counted(problem_bank_size)) {
                char const problem_difficulty = from_cin();
                result[problem_difficulty]++;
            }
            return result;
        }();
        unsigned problem_missing_count = 0u;
        for(auto const [problem_difficulty, problem_count] : problem_bank) {
            if(problem_count < round_count)
                problem_missing_count += (round_count - problem_count);
        }
        std::cout << problem_missing_count << std::endl;
    }
}
