...
// Problem statement: https://codeforces.com/problemset/problem/1296/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ...
        unsigned const odd_count = std::ranges::count_if(numbers, [](unsigned x) { return x % 2u == 1u; });
        unsigned const even_count = number_count - odd_count;
        bool const transform_possible = [odd_count, even_count]() {
            if(odd_count % 2u == 1u) return true;
            if(odd_count == 0u) return false;
            if(even_count > 0u) return true;
            return false;
        }();
        std::cout << (transform_possible ? "YES" : "NO") << std::endl;
    }
}
