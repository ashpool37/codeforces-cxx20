...
// Problem statement: https://codeforces.com/problemset/problem/1542/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const half_number_count = from_cin();
        unsigned const number_count = half_number_count * 2u;
        std::vector<unsigned> const numbers = from_cin(number_count);
        unsigned const odd_count = static_cast<unsigned>(
            std::ranges::count_if(numbers, [](unsigned x) { return x % 2u != 0u; }));
        bool const splitting_possible = odd_count * 2u == number_count;
        std::cout << (splitting_possible ? "YES" : "NO") << std::endl;
    }
}
