...
// Problem statement: https://codeforces.com/problemset/problem/1676/B?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        unsigned const box_count = from_cin();
        std::vector<umax> const candy_counts = from_cin(box_count);
        umax const min_candy_count = std::ranges::min(candy_counts);

        auto const eaten_candy_counts =
            std::views::transform(
                candy_counts,
                [min_candy_count](umax candy_count) {
                    return candy_count - min_candy_count;
                });
        umax const eaten_candy_total_count = std::accumulate(eaten_candy_counts.begin(),
                                                             eaten_candy_counts.end(),
                                                             0u);
        std::cout << eaten_candy_total_count << std::endl;
    }
}
