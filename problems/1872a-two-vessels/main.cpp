// Problem: 1872A. Two Vessels
// Problem statement: https://codeforces.com/problemset/problem/1872/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const first_vessel_mass = from_cin();
        unsigned const second_vessel_mass = from_cin();
        unsigned const cup_capacity = from_cin();

        unsigned const difference = abs_sub(first_vessel_mass, second_vessel_mass);
        unsigned const move_count = ceil_div(difference, 2u * cup_capacity);
