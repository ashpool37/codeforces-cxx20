...
// Problem statement: https://codeforces.com/problemset/problem/1760/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        [[maybe_unused]] unsigned const message_length = from_cin();
        cin_skip_ws();
        std::string const message = line_from_cin();
        char const max_char = std::ranges::max(message);
        unsigned const alphabet_size = static_cast<unsigned>(max_char - 'a') + 1u;
