...
// Problem statement: https://codeforces.com/problemset/problem/702/A?locale=en
...
int main() {
    ...
    unsigned last_number = from_cin();
    unsigned increasing_subseq_length = 1u;
    unsigned increasing_subseq_length_max = 1u;
    for(auto const _ : counted(number_count - 1u)) {
        unsigned const number = from_cin();
        if(number > last_number) {
            increasing_subseq_length++;
            increasing_subseq_length_max = std::max(increasing_subseq_length, increasing_subseq_length_max);
        }
        else {
            increasing_subseq_length = 1u;
        }
        last_number = number;
    }

    std::cout << increasing_subseq_length_max << std::endl;
}
