...
// Problem statement: https://codeforces.com/problemset/problem/1669/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ...
        std::unordered_map<unsigned, unsigned> number_counts;
        auto const triple = [&numbers, &number_counts]() -> std::optional<unsigned> {
            for(auto const number : numbers) {
                number_counts[number]++;
                if(number_counts.at(number) >= 3) return number;
            }
            return std::nullopt;
        }();
        std::cout << (triple.has_value() ? static_cast<int>(*triple) : -1) << std::endl;
    }
}
