...
// Problem statement: https://codeforces.com/problemset/problem/707/A?locale=en
...
int main() {
    std::size_t const photo_width = from_cin();
    std::size_t const photo_height = from_cin();
    std::size_t const pixel_count = photo_width * photo_height;
    enum AnalyzeColorsResult {
        GRAYSCALE,
        COLOR,
    };
    auto const analyze_colors = [pixel_count]() {
        std::unordered_set<std::string> const colors = {"C", "M", "Y"};
        for(auto _ : counted(pixel_count)) {
            std::string const pixel_value = word_from_cin();
            if(colors.contains(pixel_value)) return AnalyzeColorsResult::COLOR;
        }
        return AnalyzeColorsResult::GRAYSCALE;
    };
    std::cout << (analyze_colors() == AnalyzeColorsResult::COLOR ? "#Color" : "#Black&White") << std::endl;
}
