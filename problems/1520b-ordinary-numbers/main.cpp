...
// Problem statement: https://codeforces.com/problemset/problem/1520/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ull const upper_bound = from_cin();
        ull const digit_count = static_cast<ull>(std::floor(std::log10(upper_bound))) + 1u;
        ull const whole_set_count = digit_count - 1u;
        ull const nth_eleventy = (power(10u, digit_count) - 1u) / 9u;
        ull const last_set_size = upper_bound / nth_eleventy;
        ull const total_count = whole_set_count * 9u + last_set_size;
