...
// Problem statement: https://codeforces.com/problemset/problem/270/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const interior_angle = from_cin();
        unsigned const exterior_angle = 180u - interior_angle;
        bool const regular = 360u % exterior_angle == 0u;
        std::cout << (regular ? "YES" : "NO") << std::endl;
    }
}
