...
// Problem statement: https://codeforces.com/problemset/problem/749/A?locale=en
...
int main() {
    unsigned number = from_cin();
    bool even = number % 2u == 0;
    if(even) {
        unsigned twos_count = number / 2u;
        std::cout << twos_count << std::endl;
        for(auto _ : counted(twos_count - 1)) std::cout << 2 << " ";
        std::cout << 2 << std::endl;
    }
    else {
        unsigned twos_count = (number - 3u) / 2u;
        std::cout << twos_count + 1u << std::endl;
        for(auto _ : counted(twos_count)) std::cout << 2 << " ";
        std::cout << 3 << std::endl;
    }
}
