...
// Problem statement: https://codeforces.com/problemset/problem/1619/A?locale=en
...
bool is_square (std::string const& string) {
    if(string.size() % 2 != 0) return false;
    std::size_t half_length = string.size() / 2;

    return std::ranges::equal(std::views::take(string, half_length),
                              std::views::drop(string, half_length));
};
...
int main() {
    ...
    cin_skip_line();
    while(test_count--) {
        std::string string = from_cin();
        std::cout << (is_square(string) ? "YES" : "NO") << std::endl;
    }
}
