...
// Problem statement: https://codeforces.com/problemset/problem/742/A?locale=en
...
int main() {
    unsigned const exponent = from_cin();
    std::array<unsigned, 4> const digits = {6, 8, 4, 2};
    if(exponent == 0u) std::cout << 1u << std::endl;
    else std::cout << digits[exponent % 4u] << std::endl;
}
