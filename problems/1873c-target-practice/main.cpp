...
// Problem statement: https://codeforces.com/problemset/problem/1873/C?locale=en
...
unsigned ring_on_equator(unsigned const coord) {
    std::array<unsigned, 10uz> const rings = {0, 1, 2, 3, 4, 4, 3, 2, 1, 0};
    return rings[coord];
}

unsigned points_by_ring(unsigned const ring) {
    return ring + 1u;
}

unsigned points_by_coords(unsigned const x, unsigned const y) {
    unsigned ring = std::min(ring_on_equator(x), ring_on_equator(y));
    return points_by_ring(ring);
}
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned total_points = 0u;
        for(unsigned y = 0u; y < 10u; y++) {
            for(unsigned x = 0u; x < 10u; x++) {
                char const hit = from_cin();
                if(hit == 'X') total_points += points_by_coords(x, y);
            }
            cin_skip_line();
        }
