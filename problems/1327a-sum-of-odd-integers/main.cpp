...
// Problem statement: https://codeforces.com/problemset/problem/1327/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ull const number = from_cin();
        ull const summand_count = from_cin();
        bool const representable = [number, summand_count]() {
            if(summand_count % 2ull != number % 2ull) return false;
            ull const min_number = (2ull * 1ull + (summand_count - 1ull) * 2ull) * summand_count / 2ull;
            if(number < min_number) return false;
            return true;
        }();
        std::cout << (representable ? "YES" : "NO") << std::endl;
    }
}
