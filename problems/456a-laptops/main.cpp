...
// Problem statement: https://codeforces.com/problemset/problem/456/A?locale=en
...
std::istream& operator>>(std::istream& is, Laptop& laptop) {
    is >> laptop.price >> laptop.quality;
    return is;
}

int main() {
    std::size_t const laptop_count = from_cin();
    std::vector<Laptop> const laptops_by_price_asc = [laptop_count]() {
        std::vector<Laptop> laptops = from_cin(laptop_count);
        auto const laptop_price_lt = [](Laptop const& lhs, Laptop const& rhs) {
            return lhs.price < rhs.price;
        };
        std::ranges::sort(laptops, laptop_price_lt);
        return laptops;
    }();
    auto const laptop_quality_lt = [](Laptop const& lhs, Laptop const& rhs) {
        return lhs.quality < rhs.quality;
    };
    bool const quality_is_ascending = std::ranges::is_sorted(laptops_by_price_asc, laptop_quality_lt);
    std::cout << (quality_is_ascending ? "Poor Alex" : "Happy Alex") << std::endl;
}
