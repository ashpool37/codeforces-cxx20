...
// Problem statement: https://codeforces.com/problemset/problem/2000/B?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        unsigned const seat_count = from_cin();
        auto const rules_followed = [seat_count]() {
            std::vector<bool> seats_taken(seat_count, false);
            for(size_t passenger_number : counted(seat_count)) {
                unsigned const seat = from_cin();
                unsigned const seat_id = seat - 1u;
                unsigned const final_seat_id = seat_count - 1u;
                bool passenger_followed_rules =
                    passenger_number == 0
                    or (seat_id > 0 and seats_taken[seat_id - 1u])
                    or (seat_id < final_seat_id and seats_taken[seat_id + 1u]);
                if(not passenger_followed_rules) {
                    cin_skip_line();
                    return false;
                }
        ...
        std::cout << (rules_followed() ? "YES" : "NO") << std::endl;
    }
}
