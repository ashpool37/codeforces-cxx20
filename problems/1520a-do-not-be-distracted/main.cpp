...
// Problem statement: https://codeforces.com/problemset/problem/1520/A?locale=en
...
int main() {
    ...
    while(test_count--) {
        std::size_t day_count = from_cin();
        cin_skip_line();
        std::unordered_set<char> started;

        char last_task = from_cin();
        started.insert(last_task);

        auto distracted = [day_count, &started, &last_task]() -> bool {
            for(auto _ : counted(day_count - 1)) {
                char task = from_cin();
                if(started.contains(task) and last_task != task)
                    return true;
                last_task = task;
                started.insert(task);
            }
        ...
        std::cout << (distracted() ? "NO" : "YES") << std::endl;
        cin_skip_line();
    }
}
