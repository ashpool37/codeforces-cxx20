...
// Problem statement: https://codeforces.com/problemset/problem/233/A?locale=en
...
int main() {
    unsigned const permutation_size = from_cin();
    auto const perfect_permutation = [permutation_size]() -> std::optional<std::vector<unsigned>> {
        if(permutation_size % 2 != 0) return std::nullopt;
        else return std::views::iota(1u, permutation_size + 1u) | std::views::reverse | std::ranges::to<std::vector<unsigned>>();
    }();
    if(perfect_permutation.has_value()) {
        for(auto i : *perfect_permutation) std::cout << i << " ";
        std::cout << std::endl;
    }
