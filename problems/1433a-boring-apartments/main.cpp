...
// Problem statement: https://codeforces.com/problemset/problem/1433/A?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned final_apartment = from_cin();

        unsigned total_digit_count = 0;
        for(auto digit : counted(1u, 9)) {
            for(auto digit_count : counted(1u, 4)) {
                total_digit_count += digit_count;
                unsigned apartment = (power(10u, digit_count) - 1u) / 9u * digit;
                if(apartment == final_apartment) goto done;
            }
        }
        done: std::cout << total_digit_count << std::endl;
    }
}
