...
// Problem statement: https://codeforces.com/problemset/problem/379/A?locale=en
...
int main() {
    unsigned const initial_candle_count = from_cin();
    unsigned const used_candle_count_for_new_candle = from_cin();

    unsigned candle_count = initial_candle_count;
    unsigned used_candle_count = 0;
    unsigned hour_count = 0;
    while(candle_count > 0 or used_candle_count >= used_candle_count_for_new_candle) {
        hour_count += candle_count;
        used_candle_count += candle_count;
        candle_count = 0;

        unsigned const new_candle_count =
            used_candle_count / used_candle_count_for_new_candle;
        candle_count += new_candle_count;
        used_candle_count -= new_candle_count * used_candle_count_for_new_candle;
    }

    std::cout << hour_count << std::endl;
}
