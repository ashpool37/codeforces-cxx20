...
// Problem statement: https://codeforces.com/problemset/problem/977/B?locale=en
...
int main() {
    unsigned const string_length = from_cin();
    cin_skip_line();
    std::string const string = line_from_cin();
    std::unordered_map<std::string, unsigned> two_gram_occurence_counts;
    unsigned two_gram_occurence_count_max = 0u;
    std::string two_gram_max;
    for(unsigned i = 0; i < string_length - 1u; i++) {
        std::string const substr = string.substr(i, 2uz);
        two_gram_occurence_counts[substr]++;
        if(two_gram_occurence_counts.at(substr) > two_gram_occurence_count_max) {
            two_gram_occurence_count_max = two_gram_occurence_counts.at(substr);
            two_gram_max = substr;
        }
    }
    std::cout << two_gram_max << std::endl;
}
