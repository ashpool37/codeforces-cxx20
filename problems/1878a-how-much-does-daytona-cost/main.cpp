...
// Problem statement: https://codeforces.com/problemset/problem/1878/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ...
        std::vector<unsigned> const array = from_cin(array_length);
        bool const number_in_array = std::ranges::contains(array, number);
        std::cout << (number_in_array ? "YES" : "NO") << std::endl;
    }
}
