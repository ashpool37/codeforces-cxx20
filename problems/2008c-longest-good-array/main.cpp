...
// Problem statement: https://codeforces.com/problemset/problem/2008/C?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const lower_bound = from_cin();
        unsigned const upper_bound = from_cin();
        unsigned const max_element = upper_bound - lower_bound + 1u;
        unsigned const max_element_index =
            static_cast<unsigned>(
                std::floor((1.0 + std::sqrt(8.0 * static_cast<double>(max_element) - 7.0)) / 2.0)
            );
