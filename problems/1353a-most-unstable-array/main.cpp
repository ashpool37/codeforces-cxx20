...
// Problem statement: https://codeforces.com/problemset/problem/1353/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const array_size = from_cin();
        unsigned const array_sum = from_cin();
        ull const max_adjacent_sum = [array_size, array_sum]() {
            if(array_size == 1u) return 0ull;
            if(array_size == 2u) return static_cast<ull>(array_sum);
            return static_cast<ull>(array_sum) * 2ull;
        }();
        std::cout << max_adjacent_sum << std::endl;
    }
}
