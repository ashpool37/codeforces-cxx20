...
// Problem statement: https://codeforces.com/problemset/problem/478/B?locale=en
...
ull choose2(ull total_count) {
    return total_count * (total_count - 1u) / 2u;
}

int main() {
    ull const participant_count = from_cin();
    ull const team_count = from_cin();

    ull const pair_count_max = [participant_count, team_count]() {
        ull const big_team_size = participant_count - (team_count - 1ull);
        ull const big_team_pair_count = choose2(big_team_size);
        return big_team_pair_count;
    }();

    ull const pair_count_min = [participant_count, team_count]() {
        auto const div_result = std::div(static_cast<ll>(participant_count), static_cast<ll>(team_count));
        ull const team_size_regular = static_cast<ull>(div_result.quot);
        ull const team_size_extended = team_size_regular + 1ull;
        ull const extended_team_count = static_cast<ull>(div_result.rem);
        ull const regular_team_count = team_count - extended_team_count;
        return extended_team_count * choose2(team_size_extended) + regular_team_count * choose2(team_size_regular);
    }();

    std::cout << pair_count_min << " " << pair_count_max << std::endl;
}
