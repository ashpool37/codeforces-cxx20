...
// Problem statement: https://codeforces.com/problemset/problem/451/B?locale=en
...
int main() {
    std::size_t const number_count = from_cin();
    std::vector<unsigned> const numbers = from_cin(number_count);
    auto const number = [&numbers](std::size_t index) -> unsigned {
        return numbers[index - 1uz];
    };
    auto const prev_number = [&numbers](std::size_t index) -> unsigned {
        return numbers[index - 2uz];
    };
    auto const next_number = [&numbers](std::size_t index) -> unsigned {
        return numbers[index];
    };
    auto const find_nontriv_dec_subsegment = [number, next_number]
            (Segment parent_segment) -> std::optional<Segment> {
        std::size_t lower_bound = parent_segment.lower_bound;
        while(lower_bound + 1uz <= parent_segment.upper_bound
                and next_number(lower_bound) > number(lower_bound))
            lower_bound++;
        std::size_t upper_bound = lower_bound;
        while(upper_bound + 1uz <= parent_segment.upper_bound
                and next_number(upper_bound) < number(upper_bound))
            upper_bound++;
        if(lower_bound == upper_bound) return std::nullopt;
        else return Segment{lower_bound, upper_bound};
    };
    auto const segment_to_reverse = [find_nontriv_dec_subsegment, number_count, number, next_number, prev_number]()
            -> std::optional<Segment> {
        auto const first_dec_subsegment = find_nontriv_dec_subsegment({1uz, number_count});
        if(not first_dec_subsegment.has_value()) return Segment{1uz, 1uz};
        if(first_dec_subsegment->lower_bound > 1uz
            and prev_number(first_dec_subsegment->lower_bound) > number(first_dec_subsegment->upper_bound))
            return std::nullopt;
        if(first_dec_subsegment->upper_bound == number_count)
            return *first_dec_subsegment;
        auto const remainder = Segment{first_dec_subsegment->upper_bound + 1uz, number_count};
        auto const next_dec_subsegment = find_nontriv_dec_subsegment(remainder);
        if(next_dec_subsegment.has_value()) return std::nullopt;
        if(next_number(first_dec_subsegment->upper_bound) < number(first_dec_subsegment->lower_bound))
            return std::nullopt;
        return first_dec_subsegment;
    }();
    if(segment_to_reverse.has_value()) {
        std::cout << "yes\n" << segment_to_reverse->lower_bound << " " << segment_to_reverse->upper_bound << std::endl;
    }
