...
// Problem statement: https://codeforces.com/problemset/problem/489/B?locale=en
...
int main() {
    using Skills = std::vector<unsigned>;
    unsigned const boy_count = from_cin();
    Skills boy_skills = from_cin(boy_count);
    unsigned const girl_count = from_cin();
    Skills girl_skills = from_cin(girl_count);

    std::ranges::sort(boy_skills);
    std::ranges::sort(girl_skills);

    struct Match {
        Skills::const_iterator boy_it;
        Skills::const_iterator girl_it;
    };
    auto find_match = [&boy_skills = std::as_const(boy_skills),
                       &girl_skills = std::as_const(girl_skills)]() 
                      ->  std::optional<Match> {
        for(auto boy_it = boy_skills.begin(); boy_it != boy_skills.end(); boy_it++) {
            for(auto girl_it = girl_skills.begin(); girl_it != girl_skills.end(); girl_it++) {
                if(abs_diff(*boy_it, *girl_it) <= 1) 
                    return Match{boy_it, girl_it};
                else if(*girl_it > *boy_it and abs_diff(*boy_it, *girl_it) > 1)
                    break;
            }
        }
        return std::nullopt;
    ...
    while((match = find_match()).has_value()) {
        match_count++;
        boy_skills.erase(match->boy_it);
        girl_skills.erase(match->girl_it);
    }
