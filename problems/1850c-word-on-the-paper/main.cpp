...
// Problem statement: https://codeforces.com/problemset/problem/1850/C?locale=en
...
bool letter_p(char ch) {
    return static_cast<bool>(std::isalpha(static_cast<int>(ch)));
}

std::optional<char> first_letter(std::string const& string) {
    auto const ch_it = std::ranges::find_if(string, letter_p);
    if(ch_it != string.end()) return *ch_it;
...
int main() {
    ...
    cin_skip_line();
    unsigned const line_count = 8u;
    for(auto const _ : counted(test_count)) {
        std::string result;
        for(auto const _ : counted(line_count)) {
            std::string const line = line_from_cin();
            auto const letter = first_letter(line);
            if(letter.has_value()) result.push_back(*letter);
        }
