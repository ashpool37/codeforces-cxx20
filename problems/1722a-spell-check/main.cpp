...
// Problem statement: https://codeforces.com/problemset/problem/1722/A?locale=en
...
int main() {
    ...
    std::string const name = "Timur";
    cin_skip_line();
    while(test_count--) {
        [[maybe_unused]] unsigned string_length = from_cin();
        cin_skip_line();
        std::string string = from_cin();
        bool string_is_valid = std::ranges::is_permutation(name, string);
        std::cout << (string_is_valid ? "YES" : "NO") << std::endl;
    }
}
