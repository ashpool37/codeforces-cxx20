...
// Problem statement: https://codeforces.com/problemset/problem/1342/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ull const number1 = from_cin();
        ull const number2 = from_cin();
        ull const cost_dec_one = from_cin();
        ull const cost_dec_both = from_cin();
        ull const optimal_cost_dec_both = std::min(cost_dec_both, 2ull * cost_dec_one);
        ull const dec_one_count = static_cast<ull>(
            std::abs(static_cast<ll>(number1) - static_cast<ll>(number2))
        );
        ull const dec_both_count = std::min(number1, number2);
        ull const result = dec_one_count * cost_dec_one + dec_both_count * optimal_cost_dec_both;
