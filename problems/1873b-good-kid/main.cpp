...
// Problem statement: https://codeforces.com/problemset/problem/1873/B?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        unsigned number_count = from_cin();
        std::vector<unsigned> numbers = from_cin(number_count);
        auto min_number_it = std::ranges::min_element(numbers);
        unsigned product = 1;
        for(auto number_it = numbers.begin(); number_it != numbers.end(); number_it++) {
            unsigned number = number_it == min_number_it ? *number_it + 1 : *number_it;
            product *= number;
        }
