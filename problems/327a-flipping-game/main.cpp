...
// Problem statement: https://codeforces.com/problemset/problem/327/A?locale=en
...
requires std::is_signed_v<std::ranges::range_value_t<R>>
auto max_subrange_sum(R&& range) {
    if(range.empty()) throw std::domain_error("Empty ranges are not admitted");
    using I = std::ranges::range_value_t<R>;
    I max_sum_at_prev_index = *std::ranges::begin(range);
    I max_sum_ever = max_sum_at_prev_index;
    for(auto it = std::ranges::next(std::ranges::begin(range));
            it != std::ranges::end(range);
            it++) {
        max_sum_at_prev_index = std::max(max_sum_at_prev_index + *it, *it);
        max_sum_ever = std::max(max_sum_ever, max_sum_at_prev_index);
    }
    return max_sum_ever;
}

int main() {
    unsigned const number_count = from_cin();
    std::vector<int> const numbers = from_cin(number_count);
    std::vector<int> const gains = numbers
        | std::views::transform([](int x){ return (x == 1 ? -1 : 1); })
        | std::ranges::to<std::vector<int>>();
    int const one_count = std::ranges::count_if(numbers, [](int x) { return x == 1; });
    int const max_gain = max_subrange_sum(gains);
    int const one_count_after_move = one_count + max_gain;
    std::cout << one_count_after_move << std::endl;
}
