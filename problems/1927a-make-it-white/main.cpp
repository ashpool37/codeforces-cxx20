...
// Problem statement: https://codeforces.com/problemset/problem/1927/A?locale=en
...
std::istream& operator>>(std::istream& input_stream, Colour& colour) {
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const strip_length = from_cin();
        std::vector<Colour> strip = from_cin(strip_length);
        unsigned const white_prefix_length = static_cast<unsigned>(std::ranges::find(strip, Colour::BLACK) - strip.begin());
        unsigned const white_suffix_length = static_cast<unsigned>(
            std::ranges::find(strip.rbegin(), strip.rend(), Colour::BLACK) - strip.rbegin());
        unsigned const non_white_substrip_length = strip_length - white_prefix_length - white_suffix_length;
        std::cout << non_white_substrip_length << std::endl;
    }
}
