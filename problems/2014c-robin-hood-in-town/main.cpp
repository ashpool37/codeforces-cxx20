// Problem: 2014C. Robin Hood in Town
// Problem statement: https://codeforces.com/problemset/problem/2014/C?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ll const population = from_cin();
        std::vector<ll> const wealth_by_person = [population]() {
            std::vector<ll> result = from_cin(population);
            std::ranges::nth_element(result, result.begin() + population / 2ll);
            return result;
        }();
        if(population < 3ll) std::cout << -1 << std::endl;
        else {
            ll const median = wealth_by_person[population / 2ll];
            ll min_pot = static_cast<ll>(2u * population * median)
                        - static_cast<ll>(sum(wealth_by_person));
            std::cout << std::max(0ll, min_pot + 1ll) << std::endl;
        }
    }
}
