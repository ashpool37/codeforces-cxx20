// Problem: 34B. Sale
// Problem statement: https://codeforces.com/problemset/problem/34/B?locale=en
...
int main() {
    unsigned tv_count = from_cin();
    std::size_t carrying_capacity = from_cin();
    std::vector<int> tv_prices = from_cin(tv_count);
    std::vector<int> tv_prices_negative;
    std::ranges::copy_if(tv_prices, std::back_inserter(tv_prices_negative),
                            [](int price) { return price < 0; });
    std::ranges::sort(tv_prices_negative);
    std::size_t carry_count = std::min(carrying_capacity, tv_prices_negative.size());
    int profit = -1 * std::accumulate(tv_prices_negative.begin(),
                                      tv_prices_negative.begin() + carry_count,
                                      0);
