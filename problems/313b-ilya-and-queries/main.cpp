...
// Problem statement: https://codeforces.com/problemset/problem/313/B?locale=en
...
int main() {
    ...
    std::vector<unsigned> const prefix_sums = [&string]() {
        std::vector<unsigned> result;
        result.reserve(string.size() - 1uz);
        unsigned running_sum = 0;
        for(auto it = string.begin(); std::next(it) != string.end(); it++) {
            if(*it == *std::next(it)) running_sum++;
            result.push_back(running_sum);
        }
        return result;
    }();
    unsigned const query_count = from_cin();
    for(auto const _ : counted(query_count)) {
        std::size_t const lower_bound = from_cin();
        std::size_t const upper_bound = from_cin();
        unsigned const answer = prefix_sums[upper_bound - 2uz]
            - (lower_bound > 1uz ? prefix_sums[lower_bound - 2uz] : 0u);
