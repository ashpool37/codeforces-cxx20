// Problem: 2022B. Kar Salesman
// Problem statement: https://codeforces.com/problemset/problem/2022/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ull const model_count = from_cin();
        ull const max_lot_size = from_cin();
        ull max_cars_of_same_model = 0u;
        ull total_car_count = 0u;
        for(auto const _ : counted(model_count)) {
            ull const car_count = from_cin();
            total_car_count += car_count;
            max_cars_of_same_model = std::max(max_cars_of_same_model, car_count);
        }
        std::cout << std::max(ceil_div(total_car_count, max_lot_size), max_cars_of_same_model) << std::endl;
    }
}
