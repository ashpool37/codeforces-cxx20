...
// Problem statement: https://codeforces.com/problemset/problem/1579/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        std::string const init_string = from_cin();
        std::istringstream ss(init_string);
        unsigned a_count = 0u;
        unsigned b_count = 0u;
        unsigned c_count = 0u;
        char c;
        ...
        std::cout << (b_count == a_count + c_count ? "YES" : "NO") << std::endl;
    }
}
