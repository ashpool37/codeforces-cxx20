...
// Problem statement: https://codeforces.com/problemset/problem/1462/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ...
        auto const mangled_index = [number_count](unsigned const index) {
            if(index % 2u == 0u) return index / 2u;
            else return number_count - 1u - index / 2u;
        };
        for(auto const i : counted(number_count)) {
            unsigned new_index = mangled_index(i);
            std::cout << numbers[new_index] << " ";
        }
        std::cout << std::endl;
    }
}
