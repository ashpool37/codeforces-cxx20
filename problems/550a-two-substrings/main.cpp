...
// Problem statement: https://codeforces.com/problemset/problem/550/A?locale=en
...
int main() {
    std::string const string = line_from_cin();
    auto const string_view = std::string_view(string);
    std::size_t const size = string_view.size();
    auto const substr = [&string_view](std::size_t const i) {
        return string_view.substr(i, 2uz);
    };
    bool const found = [size, substr]() {
        std::optional<std::size_t> ab_seen;
        std::optional<std::size_t> ba_seen;
        for(std::size_t i = 0; i + 1uz < size; i++) {
            if(substr(i) == "AB") {
                if(not ab_seen.has_value()) ab_seen = i;
                if(ba_seen.has_value() and (i - *ba_seen) > 1uz) return true;
            }
            if(substr(i) == "BA") {
                if(not ba_seen.has_value()) ba_seen = i;
                if(ab_seen.has_value() and (i - *ab_seen) > 1uz) return true;
            }
        }
        return false;
    }();
