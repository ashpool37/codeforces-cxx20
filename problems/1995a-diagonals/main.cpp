...
// Problem statement: https://codeforces.com/problemset/problem/1995/A?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned board_width = from_cin();
        unsigned chip_count = from_cin();

        unsigned diagonal_size = board_width;
        unsigned taken_diagonal_count = 0;
        unsigned diagonals_of_size_left = 1;
        while(chip_count > 0 and diagonal_size > 0) {
            chip_count -= std::min(chip_count, diagonal_size);
            taken_diagonal_count++;
            diagonals_of_size_left--;
            if(diagonals_of_size_left == 0) {
                diagonal_size--;
                diagonals_of_size_left = 2;
            }
        }
        std::cout << taken_diagonal_count << std::endl;
    }
}
