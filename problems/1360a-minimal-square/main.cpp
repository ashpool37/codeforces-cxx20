...
// Problem statement: https://codeforces.com/problemset/problem/1360/A?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned small_side = from_cin();
        unsigned big_side = from_cin();
        if(small_side > big_side) std::swap(small_side, big_side);
        unsigned square_side = std::max(big_side, 2u * small_side);
        unsigned square_area = square_side * square_side;
