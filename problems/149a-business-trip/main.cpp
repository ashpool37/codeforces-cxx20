...
// Problem statement: https://codeforces.com/problemset/problem/149/A?locale=en
...
int main() {
    unsigned const min_growth = from_cin();
    std::vector<unsigned> const growth_by_month = []() {
        std::vector<unsigned> result = from_cin(12uz);
        std::ranges::sort(result, std::greater());
        return result;
    }();
    auto const month_count = [min_growth, &growth_by_month]() -> std::optional<unsigned> {
        unsigned growth = 0u;
        unsigned result = 0u;
        if(min_growth == 0u) return result;
        for(auto const growth_in_month : growth_by_month) {
            growth += growth_in_month;
            result += 1u;
            if(growth >= min_growth) return result;
        }
        return std::nullopt;
    }();
    std::cout << print_opt(month_count, -1) << std::endl;
}
