...
// Problem statement: https://codeforces.com/problemset/problem/1348/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        ull const n = from_cin();
        ull lhs = 0ull, rhs = 0ull;
        lhs += power(2ull, n);
        for(ull i = 1ull; i <= n/2ull - 1ull; i++) {
            lhs += power(2ull, i);
        }
        for(ull i = n/2ll; i <= n - 1ull; i++) {
            rhs += power(2ull, i);
        }
