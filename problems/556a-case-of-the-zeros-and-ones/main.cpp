...
// Problem statement: https://codeforces.com/problemset/problem/556/A?locale=en
...
int main() {
    unsigned const initial_string_length = from_cin();
    cin_skip_line();
    std::list<char> string = from_cin(initial_string_length);
    unsigned deletion_count = 0u;
    for(auto it = string.begin(); it != string.end();) {
        if(std::next(it) != string.end()
                and ((*it == '0' and *std::next(it) == '1')
                        or (*it == '1' and *std::next(it) == '0'))) {
            if(it != string.begin()) {
                std::advance(it, -1);
                string.erase(std::next(it), std::next(it, 3));
            }
            else {
                string.erase(it, std::next(it, 2));
                it = string.begin();
            }
            deletion_count += 1;
        }
    ...
    std::cout << initial_string_length - deletion_count * 2u << std::endl;
}
