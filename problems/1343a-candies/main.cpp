...
// Problem statement: https://codeforces.com/problemset/problem/1343/A?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        unsigned const candy_count = from_cin();
        auto calculate_formula = [candy_count]() -> std::optional<unsigned> {
            for(unsigned k = 2; k <= 29; k++) {
                unsigned divisor = power(2u, k) - 1u;
                if(candy_count % divisor == 0) return candy_count / divisor;
            }
            return std::nullopt;
        };
        std::cout << *calculate_formula() << std::endl;
    }
}
