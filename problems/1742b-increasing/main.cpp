...
// Problem statement: https://codeforces.com/problemset/problem/1742/B?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        std::size_t const array_length = from_cin();
        std::unordered_set<unsigned> elements_seen;
        auto all_elements_unique_p = [&elements_seen, array_length]() {
            for(auto _ : counted(array_length)) {
                unsigned const element = from_cin();
                auto const insert_result = elements_seen.insert(element);
                bool const insertion_took_place = insert_result.second;
                if(not insertion_took_place) {
                    cin_skip_line();
                    return false;
                }
            }
        ...
        std::cout << (all_elements_unique_p() ? "YES" : "NO") << std::endl;
    }
}
