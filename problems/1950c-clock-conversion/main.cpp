...
// Problem statement: https://codeforces.com/problemset/problem/1950/C?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const hours = from_cin();
        (void) std::cin.get();
        unsigned const minutes = from_cin();
        unsigned const hours12 = [hours]() {
            unsigned const hours_mod_12 = hours % 12;
            if(hours_mod_12 == 0u) return 12u;
            else return hours_mod_12;
        }();
        std::string const ampm = hours < 12 ? "AM" : "PM";
        std::cout << std::setfill('0')
                  << std::setw(2) << hours12
                  << std::setw(1) << ':'
                  << std::setw(2) << minutes
                  << std::setw(1) << ' ' << ampm << std::endl;
    }
}
