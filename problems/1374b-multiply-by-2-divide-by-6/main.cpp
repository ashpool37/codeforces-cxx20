...
// Problem statement: https://codeforces.com/problemset/problem/1374/B?locale=en
...
int main() {
    ...
    while(test_count--) {
        ...
        enum TryDivideNumberResult {
            DIVIDE_SUCCESS,
            DIVIDE_FAIL,
        };
        auto try_divide_number = [&number, &operation_count]() {
            if(number % 6u == 0u) {
                number = number / 6u;
                operation_count += 1;
                return DIVIDE_SUCCESS;
            }
            else if(number % 3u == 0u) {
                number = number * 2u / 6u;
                operation_count += 2;
                return DIVIDE_SUCCESS;
            }
        ...
        while(try_divide_number() == DIVIDE_SUCCESS);

        std::cout << (number == 1u ? static_cast<int>(operation_count) : -1) << std::endl;
    }
}
