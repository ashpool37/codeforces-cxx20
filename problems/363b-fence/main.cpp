// Problem: 363B. Fence
// Problem statement: https://codeforces.com/problemset/problem/363/B?locale=en
...
int main() {
    unsigned fence_length = from_cin();
    unsigned piano_width = from_cin();

    unsigned total_broken_height = 0;
    std::queue<unsigned> planks_broken;
    for([[maybe_unused]] auto _ : counted(piano_width)) {
        unsigned plank = from_cin();
        total_broken_height += plank;
        planks_broken.push(plank);
    }

    unsigned total_broken_height_min = total_broken_height;
    unsigned hole_position_min = 0;
    for(unsigned hole_position : counted(1u, fence_length - piano_width)) {
        unsigned plank = from_cin();
        total_broken_height += plank;
        planks_broken.push(plank);
        total_broken_height -= planks_broken.front();
        planks_broken.pop();

        if(total_broken_height_min > total_broken_height) {
            hole_position_min = hole_position;
            total_broken_height_min = total_broken_height;
        }
    }

    std::cout << hole_position_min + 1 << std::endl;
}
