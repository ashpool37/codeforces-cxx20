// Problem: 2008D. Sakurako's Hobby
// Problem statement: https://codeforces.com/problemset/problem/2008/D?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const array_length = from_cin();
        std::vector<unsigned> const permutation = from_cin(array_length);
        cin_skip_ws();
        std::vector<bool> const black = [array_length]() {
            std::vector<bool> result;
            result.reserve(array_length);
            for(auto const _ : counted(array_length)) {
                char const colour_ch = from_cin();
                switch(colour_ch) {
                    case '0': result.push_back(true); break;
                    default: result.push_back(false); break;
                }
            }
            return result;
        }();

        std::vector<std::optional<unsigned>> function_values(array_length, std::nullopt);
        for(unsigned i = 0; i < array_length; i++) {
            if(function_values[i].has_value()) continue;

            unsigned black_count = 0;
            unsigned j = i;
            std::vector<unsigned> cycle;
            auto const jump = [&]() {
                if(black[j]) black_count++;
                cycle.push_back(j);
                j = permutation[j] - 1u;
                return j != i;
            };
            while(jump());
            for(auto const j : cycle) {
                function_values[j] = black_count;
            }
        }
        for(unsigned i = 0; i < array_length - 1u; i++)
            std::cout << function_values[i].value() << " ";
        std::cout << function_values[array_length - 1u].value() << std::endl;
        cin_skip_ws();
    }
}
