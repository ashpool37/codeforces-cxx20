// Problem: 1996B. Scale
// Problem statement: https://codeforces.com/problemset/problem/1996/B?locale=en
...
int main() {
    ...
    while(test_count--) {
        std::size_t matrix_size = from_cin();
        std::size_t scale_factor = from_cin();
        cin_skip_line();

        std::size_t new_matrix_size = matrix_size / scale_factor;

        for(auto _ : counted(new_matrix_size)) {
            std::string line = from_cin();
            for(std::size_t i = 0; i < matrix_size; i += scale_factor) {
                std::cout << line[i];
            }
            std::cout << std::endl;
            for(auto _ : counted(scale_factor - 1)) cin_skip_line();
        }
    }
}
