...
// Problem statement: https://codeforces.com/problemset/problem/1311/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const start = from_cin();
        unsigned const goal = from_cin();
        unsigned const move_count = [start, goal]() {
            if(start == goal) return 0;
            if(goal > start and not same_parity_p(goal, start)) return 1;
            if(goal < start and same_parity_p(goal, start)) return 1;
            return 2;
        }();
