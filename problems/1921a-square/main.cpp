...
// Problem statement: https://codeforces.com/problemset/problem/1921/A?locale=en
...
std::istream& operator>>(std::istream& input_stream, Vec2D& vec) {
    input_stream >> vec.x >> vec.y;
    return input_stream;
}
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        std::array<Vec2D, 4uz> square_corners = from_cin();
        Vec2D a_minus_b = square_corners[1] - square_corners[0];
        unsigned side_length = static_cast<unsigned>(std::max(std::abs(a_minus_b.x), std::abs(a_minus_b.y)));
        unsigned area = side_length * side_length;
