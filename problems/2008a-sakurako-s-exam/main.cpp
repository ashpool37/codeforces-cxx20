...
// Problem statement: https://codeforces.com/problemset/problem/2008/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const one_count = from_cin();
        unsigned const two_count = from_cin();
        bool const solution_exists = [one_count, two_count](){
            if(one_count % 2u != 0u) return false;
            if(two_count % 2u == 0u) return true;
            if(one_count >= 2u) return true;
            return false;
        }();
        std::cout << (solution_exists ? "YES" : "NO") << std::endl;
    }
}
