...
// Problem statement: https://codeforces.com/problemset/problem/1352/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const goal_sum = from_cin();
        unsigned const summand_count = from_cin();
        auto const summands = [goal_sum, summand_count]() -> std::optional<std::vector<unsigned>> {
            if(goal_sum < summand_count) return std::nullopt;

            auto const odd_summands = [goal_sum, summand_count] -> std::optional<std::vector<unsigned>> {
                unsigned const odd_big_summand = goal_sum - (summand_count - 1u);
                if(odd_big_summand % 2u == 1u) {
                    std::vector<unsigned> result(summand_count - 1u, 1u);
                    result.push_back(odd_big_summand);
                    return result;
                }
                return std::nullopt;
            }();
            if(odd_summands.has_value()) return odd_summands;

            auto const even_summands = [goal_sum, summand_count] -> std::optional<std::vector<unsigned>> {
                if(goal_sum % 2u != 0 or summand_count * 2u > goal_sum) return std::nullopt;
                unsigned const even_big_summand = goal_sum - (summand_count - 1u) * 2u;
                std::vector<unsigned> result(summand_count - 1u, 2u);
                result.push_back(even_big_summand);
                return result;
            }();
            if(even_summands.has_value()) return even_summands;

            return std::nullopt;
        }();
        if(not summands.has_value()) std::cout << "NO" << std::endl;
        else {
            std::cout << "YES\n";
