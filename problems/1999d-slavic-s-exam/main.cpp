// Problem: 1999D. Slavic's Exam
// Problem statement: https://codeforces.com/problemset/problem/1999/D?locale=en
...
int main() {
    ...
    cin_skip_line();
    while(test_count--) {
        std::string pattern = from_cin();
        std::string const subsequence = from_cin();

        auto pattern_it = pattern.begin();
        auto subsequence_it = subsequence.begin();

        enum SeekNextMatchResult {
            MATCH_FOUND,
            END_REACHED,
        };
        auto const seek_next_match = [&pattern, &subsequence, &pattern_it, &subsequence_it]() {
            while((pattern_it != pattern.end()) and (subsequence_it != subsequence.end())) {
                if(*pattern_it == '?' or *pattern_it == *subsequence_it) {
                    *pattern_it = *subsequence_it;
                    pattern_it++;
                    subsequence_it++;
                    return SeekNextMatchResult::MATCH_FOUND;
                }
                else pattern_it++;
            }
            return SeekNextMatchResult::END_REACHED;
        };

        while(seek_next_match() == SeekNextMatchResult::MATCH_FOUND);

        bool complete_match = subsequence_it == subsequence.end();
        if(complete_match) {
