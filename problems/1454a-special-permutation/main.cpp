...
// Problem statement: https://codeforces.com/problemset/problem/1454/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const permutation_length = from_cin();
        for(unsigned i = 2u; i <= permutation_length; i++) {
            std::cout << i << " ";
        }
        std::cout << 1 << std::endl;
    }
