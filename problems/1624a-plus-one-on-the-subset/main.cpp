...
// Problem statement: https://codeforces.com/problemset/problem/1624/A?locale=en
...
int main() {
    ...
    while(test_count--) {
        std::size_t array_length = from_cin();
        std::vector<unsigned> numbers = from_cin(array_length);

        std::cout << std::ranges::max(numbers) - std::ranges::min(numbers) << std::endl;
    }
}
