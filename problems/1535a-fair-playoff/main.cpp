...
// Problem statement: https://codeforces.com/problemset/problem/1535/A?locale=en
...
std::size_t const tournament_size = 4u;

int main() {
    unsigned test_count = from_cin();
    while(test_count--) {
        std::array<unsigned, tournament_size> tournament = from_cin(tournament_size);
        auto tournament_sorted = tournament;
        std::ranges::sort(tournament_sorted, std::greater());
        unsigned winner1 = std::max(tournament[0], tournament[1]);
        unsigned winner2 = std::max(tournament[2], tournament[3]);
        if(winner1 < winner2) std::swap(winner1, winner2);
        bool tournament_is_fair = winner1 == tournament_sorted[0]
                                  and winner2 == tournament_sorted[1];
        std::cout << (tournament_is_fair ? "YES" : "NO") << std::endl;
    }
}
