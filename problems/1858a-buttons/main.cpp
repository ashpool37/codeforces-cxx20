...
// Problem statement: https://codeforces.com/problemset/problem/1858/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const anna_button_count = from_cin();
        unsigned const katie_button_count = from_cin();
        unsigned const shared_button_count = from_cin();

        Player const turn_after_shared = (shared_button_count % 2u == 0u ? Player::ANNA : Player::KATIE);
        Player const winner = [turn_after_shared, anna_button_count, katie_button_count]() {
            switch(turn_after_shared) {
                case Player::ANNA:
                    return (anna_button_count >= katie_button_count + 1u ? Player::ANNA : Player::KATIE);
                case Player::KATIE:
                    return (katie_button_count >= anna_button_count + 1u ? Player::KATIE : Player::ANNA);
            }
        }();
        std::cout << (winner == Player::ANNA ? "First" : "Second") << std::endl;
    }
}
