...
// Problem statement: https://codeforces.com/problemset/problem/1472/B?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned candy_count = from_cin();
        unsigned candy_weight_1_count = 0;
        [[maybe_unused]] unsigned candy_weight_2_count = 0;
        for(auto _ : counted(candy_count)) {
            unsigned candy_weight = from_cin();
            switch(candy_weight) {
                case 1: candy_weight_1_count++; break;
                case 2: candy_weight_2_count++; break;
            }
        }
        bool fair_division_possible = candy_weight_1_count % 2 == 0
                                      and (candy_weight_1_count >= 2
                                           or candy_weight_2_count % 2 == 0);
        std::cout << (fair_division_possible ? "YES" : "NO") << std::endl;
    }
}
