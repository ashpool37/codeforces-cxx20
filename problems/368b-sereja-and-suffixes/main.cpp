...
// Problem statement: https://codeforces.com/problemset/problem/368/B?locale=en
...
int main() {
    ...
    unsigned const request_count = from_cin();
    std::vector<unsigned> const numbers = from_cin(number_count);

    std::vector<unsigned> const different_number_counts_at_suffixes = [number_count, &numbers]() {
        std::vector<unsigned> result(number_count);
        std::unordered_set<unsigned> different_numbers_so_far;

        for(auto const suffix_start : counted(static_cast<long>(number_count - 1uz), number_count, -1l)) {
            different_numbers_so_far.insert(numbers[suffix_start]);
            result[suffix_start] = different_numbers_so_far.size();
        }

        return result;
    }();

    for(auto const _ : counted(request_count)) {
        unsigned const suffix_start = from_cin();
        std::cout << different_number_counts_at_suffixes[suffix_start - 1u] << std::endl;
    }
}
