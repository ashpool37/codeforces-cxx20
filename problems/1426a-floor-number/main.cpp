...
// Problem statement: https://codeforces.com/problemset/problem/1426/A?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        unsigned const apartment_number = from_cin();
        unsigned const floor_apartment_count = from_cin();
        unsigned const floor_number =
            apartment_number <= 2
                ? 1u
                : (apartment_number - 3u) / floor_apartment_count + 2u;
        std::cout << floor_number << std::endl;
    }
}
