...
// Problem statement: https://codeforces.com/problemset/problem/1941/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const left_pocket_size = from_cin();
        unsigned const right_pocket_size = from_cin();
        unsigned const max_sum = from_cin();
        std::vector<unsigned> left_pocket = from_cin(left_pocket_size);
        std::vector<unsigned> right_pocket = from_cin(right_pocket_size);
        unsigned combo_count = 0;
        for(auto const left_coin : left_pocket) {
            for(auto const right_coin : right_pocket) {
                if(left_coin + right_coin <= max_sum)
                    combo_count++;
            }
        }
