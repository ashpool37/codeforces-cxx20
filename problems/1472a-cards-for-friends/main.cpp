...
// Problem statement: https://codeforces.com/problemset/problem/1472/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const initial_width = from_cin();
        unsigned const initial_height = from_cin();
        unsigned const friend_count = from_cin();

        unsigned card_count = 1u;
        for(auto width = initial_width; width % 2u == 0u; width /= 2u)
            card_count *= 2u;
        for(auto height = initial_height; height % 2u == 0u; height /= 2u)
            card_count *= 2u;

        std::cout << (card_count >= friend_count ? "YES" : "NO") << std::endl;
    }
}
