...
// Problem statement: https://codeforces.com/problemset/problem/499/B?locale=en
...
int main() {
    unsigned const lecture_size = from_cin();
    unsigned const language_size = from_cin();
    auto const dictionary = [language_size]() {
        std::unordered_map<std::string, std::string> result;
        for(auto const _ : counted(language_size)) {
            std::string const word = from_cin();
            std::string const translation = from_cin();
            result[word] = translation;
        }
        return result;
    }();
    for(auto const _ : counted(lecture_size)) {
        std::string const word = from_cin();
        std::string const translation = dictionary.at(word);
        if(translation.size() < word.size()) std::cout << translation << " ";
        else std::cout << word << " ";
    }
    std::cout << std::endl;
}
