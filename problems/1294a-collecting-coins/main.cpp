...
// Problem statement: https://codeforces.com/problemset/problem/1294/A?locale=en
...
int main() {
    ...
    for(auto _ : counted(test_count)) {
        std::array<unsigned, 3> const sister_coin_counts = from_cin();
        unsigned const coin_count_to_give = from_cin();

        unsigned const sister_coin_counts_max = std::ranges::max(sister_coin_counts);
        auto const given_coin_until_even_counts =
            std::views::transform(sister_coin_counts,
                                  [sister_coin_counts_max](unsigned coin_count) {
                                        return sister_coin_counts_max - coin_count;
                                  });
        unsigned const given_coin_until_even_total_count =
            std::accumulate(given_coin_until_even_counts.begin(),
                            given_coin_until_even_counts.end(),
                            0u);
        bool const enough_coins_to_give = given_coin_until_even_total_count <= coin_count_to_give;
        bool const can_give_all_coins = enough_coins_to_give
                                        and (coin_count_to_give - given_coin_until_even_total_count) % 3 == 0;
        std::cout << (can_give_all_coins ? "YES" : "NO") << std::endl;
    }
}
