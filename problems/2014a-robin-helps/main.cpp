...
// Problem statement: https://codeforces.com/problemset/problem/2014/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const victim_count = from_cin();
        unsigned const rob_threshold = from_cin();
        unsigned gift_count = 0u;
        unsigned robin_gold = 0u;
        for(auto const _ : counted(victim_count)) {
            unsigned const victim_gold = from_cin();
            if(victim_gold >= rob_threshold) robin_gold += victim_gold;
            else if(victim_gold == 0u and robin_gold > 0u) {
                robin_gold -= 1u;
                gift_count++;
            }
        }
