...
// Problem statement: https://codeforces.com/problemset/problem/1829/A?locale=en
...
int main() {
    ...
    cin_skip_line();
    std::string const reference = "codeforces";
    while(test_count--) {
        std::string string = from_cin();
        unsigned hamming_distance = 0;
        for(auto i : counted(string.size())) {
            if(string[i] != reference[i]) hamming_distance++;
        }
