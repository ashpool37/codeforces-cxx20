...
// Problem statement: https://codeforces.com/problemset/problem/577/A?locale=en
...
int main() {
    unsigned const table_size = from_cin();
    unsigned const number_sought = from_cin();

    unsigned occurence_count = 0u;
    for(unsigned divisor = 1u; divisor <= table_size; divisor++) {
        auto div_result = std::div(static_cast<long>(number_sought), static_cast<long>(divisor));
        if(div_result.rem == 0l and static_cast<unsigned>(div_result.quot) <= table_size)
            occurence_count += 1;
    }
    std::cout << occurence_count << std::endl;
}
