...
// Problem statement: https://codeforces.com/problemset/problem/1703/B?locale=en
...
int main() {
    ...
    while(test_count--) {
        unsigned problem_count = from_cin();
        cin_skip_line();
        std::unordered_set<char> problems_solved;
        unsigned balloon_count = 0;
        for(auto _ : counted(problem_count)) {
            char problem = from_cin();
            balloon_count++;
            if(not problems_solved.contains(problem)) {
                balloon_count++;
                problems_solved.insert(problem);
            }
        }
        std::cout << balloon_count << std::endl;
    }
}
