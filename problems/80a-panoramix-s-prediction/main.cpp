...
// Problem statement: https://codeforces.com/problemset/problem/80/A?locale=en
...
int main() {
    std::vector<umax> primes;
    generate_primes(50, std::back_inserter(primes));
    unsigned first_number = from_cin();
    unsigned next_number = from_cin();
    auto first_it = std::equal_range(primes.begin(), primes.end(), first_number).first;
    std::cout << (*(first_it + 1) == next_number ? "YES" : "NO") << std::endl;
}
