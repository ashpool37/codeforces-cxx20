...
// Problem statement: https://codeforces.com/problemset/problem/500/A?locale=en
...
int main() {
    unsigned cell_count = from_cin();
    unsigned destination_cell = from_cin();
    std::vector<unsigned> portals = from_cin(cell_count - 1);

    unsigned cell = 1;
    while(cell < destination_cell) {
        cell += portals[cell - 1];
    }
    std::cout << (cell == destination_cell ? "YES" : "NO") << std::endl;
}
