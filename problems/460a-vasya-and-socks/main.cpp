...
// Problem statement: https://codeforces.com/problemset/problem/460/A?locale=en
...
int main() {
    unsigned const sock_count_initial = from_cin();
    unsigned const buy_period_length = from_cin();

    unsigned day_count = 0;
    for(unsigned sock_count = sock_count_initial; sock_count > 0; sock_count--) {
        day_count++;
        if(day_count % buy_period_length == 0) sock_count++;
    }
