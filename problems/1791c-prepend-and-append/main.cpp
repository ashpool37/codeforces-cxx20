...
// Problem statement: https://codeforces.com/problemset/problem/1791/C?locale=en
...
unsigned min_kernel(std::string const& binary_string) {
    auto min_kernel_helper = [](this auto const& helper, std::string_view binary_substring) -> std::size_t {
        bool const is_kernel = binary_substring.size() < 2uz
                               or binary_substring[0uz] == binary_substring[binary_substring.size() - 1uz];
        return is_kernel ? binary_substring.size()
                         : helper(binary_substring.substr(1uz, binary_substring.size() - 2uz));
    };
    return min_kernel_helper(std::string_view(binary_string));
}

int main() {
    unsigned const test_count = from_cin();
    for(auto const _ : counted(test_count)) {
        [[maybe_unused]] unsigned const string_length = from_cin();
        cin_skip_ws();
        std::string const binary_string = line_from_cin();
        std::cout << min_kernel(binary_string) << std::endl;
    }
}
