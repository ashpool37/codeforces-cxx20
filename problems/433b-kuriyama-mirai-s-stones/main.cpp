...
// Problem statement: https://codeforces.com/problemset/problem/433/B?locale=en
...
std::istream& operator>>(std::istream& input_stream, QuestionType& question_type) {
    std::istream::sentry sentry(input_stream);
    if(not sentry) {
    ...
    switch(input_stream.peek()) {
        case '1':
            question_type = QuestionType::ORDER_ORIGINAL;
            input_stream.get();
            break;
        case '2':
            question_type = QuestionType::ORDER_BY_PRICE;
            input_stream.get();
            break;
        default:
            input_stream.setstate(std::ios::failbit);
...
std::istream& operator>>(std::istream& input_stream, Question& question) {
    input_stream >> question.type >> question.lower_bound >> question.upper_bound;
    return input_stream;
}

int main() {
    unsigned const stone_count = from_cin();
    std::vector<ull> const stones = from_cin(stone_count);
    std::vector<ull> const stones_prefix_sums = prefix_sums(stones);
    std::vector<ull> const stones_by_price_prefix_sums = [&stones]() {
        std::vector<ull> stones_by_price = stones;
        std::ranges::sort(stones_by_price);
        return prefix_sums(stones_by_price);
    }();
    auto const subrange_sum = [](std::vector<ull> const& range, std::size_t lbound, std::size_t ubound) {
        ull const lower_prefix_sum = lbound == 0uz
            ? 0ull
            : range[lbound - 1uz];
        ull const upper_prefix_sum = range[ubound];
        return upper_prefix_sum - lower_prefix_sum;
    };
    unsigned const question_count = from_cin();
    for(auto const _ : counted(question_count)) {
        Question question = from_cin();
        switch(question.type) {
            case QuestionType::ORDER_ORIGINAL:
                std::cout << subrange_sum(stones_prefix_sums, question.lower_bound - 1uz, question.upper_bound - 1uz)
                          << std::endl;
                break;
            case QuestionType::ORDER_BY_PRICE:
                std::cout << subrange_sum(stones_by_price_prefix_sums, question.lower_bound - 1uz, question.upper_bound - 1uz)
                          << std::endl;
                break;
        }
    }
