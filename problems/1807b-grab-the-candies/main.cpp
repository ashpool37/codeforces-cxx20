...
// Problem statement: https://codeforces.com/problemset/problem/1807/B?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const bag_count = from_cin();
        std::vector<unsigned> const bags = from_cin(bag_count);
        unsigned mihai_candies = 0u;
        unsigned bianca_candies = 0u;
        for(auto const bag : bags) {
            if(bag % 2 == 0) mihai_candies += bag;
            else bianca_candies += bag;
        }
        bool const rearrangement_possible = mihai_candies > bianca_candies;
        std::cout << (rearrangement_possible ? "YES" : "NO") << std::endl;
    }
}
