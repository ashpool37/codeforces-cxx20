...
// Problem statement: https://codeforces.com/problemset/problem/1955/A?locale=en
...
int main() {
    ...
    for(auto const _ : counted(test_count)) {
        unsigned const yogurt_count = from_cin();
        unsigned const single_price = from_cin();
        unsigned const pair_price = from_cin();

        auto const div_result = std::div(static_cast<long>(yogurt_count), 2l);
        unsigned const pair_count = static_cast<unsigned>(div_result.quot);
        unsigned const single_count = static_cast<unsigned>(div_result.rem);
        unsigned const optimal_pair_price = std::min(pair_price, single_price * 2u);
        unsigned const total_cost = optimal_pair_price * pair_count + single_price * single_count;
